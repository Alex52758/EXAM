...
let API_KEY = "ddf0170a-2859-4529-adbc-f33aa50a2076";
...
function formatData(name, district, area, isNet, type, socPriv, minCapacity, maxCapacity, minDate, maxDate) {
    ...
    for (let i = 0; i < allData.length; i++) {
        ...
        // console.log("isNet type: ",typeof isNet,"; curOrg.isNet: ",typeof curOrg.isNetObject);
...
function generateId() {
    ...
    while (!idGenerated) {
        ...
        let url = new URL(API_URL + '' + randomId);
        let xhr = new XMLHttpRequest();
        xhr.open('GET', url);
        xhr.responseType = 'json';
        xhr.send();
        xhr.onload = function () {
            if (this.response.error == 'undefined') idGenerated = true;
        }
    }
    return randomId;
}
...
function createOrganization(event) {
    let id = generateId();
    // let name = document.getElementById('modalEdit--')
    // let isNetObject;
    // let operatingCompany = document.getElementById('modalEdit-input-operatingCompany').value;
    // let typeObject = document.getElementById('modalEdit-select-typeObject')
    // let district = document.getElementById('modalEdit--')
    // let admArea = document.getElementById('modalEdit--')
    // let address = document.getElementById('modalEdit--')
    // let seatsCount = document.getElementById('modalEdit--')
}
...
function createRestaurantElement(restaurantObject) {
    ...
    // infoLink.parentElement.parentElement.value = restaurantObject.id;
    // editLink.parentElement.parentElement.value = restaurantObject.id;
    // deleteLink.parentElement.parentElement.value = restaurantObject.id;

...
let order_options = [{ option: "В два раза больше", value: "+60%", isActive: false }, { option: "Хочу подарок!", value: "+1 случайный сет", isActive: false }];
let order_cost = 0;
...
// TODO TODO TODO
// Обновление цены заказа после выбора другого заведения 
// Либо обнулить, либо посчитать заново
...
function searchOrganizations(event) {
    ...
    switch (socPrivSelector[socPrivSelector.selectedIndex].value) {
        case 'Да':
            socPrivValue = "1"; // ИЗМЕНИЛ НА СТРОКУ
            break;
        case 'Нет':
            socPrivValue = "0"; // ИЗМЕНИЛ НА СТРОКУ
            break;
        case '':
            socPrivValue = "";
    }
...
function incrementSets(updateBy) {
    order_set1.count += updateBy
    order_set2.count += updateBy
    order_set3.count += updateBy
    order_set4.count += updateBy
    order_set5.count += updateBy
    order_set6.count += updateBy
    order_set7.count += updateBy
    order_set8.count += updateBy
    order_set9.count += updateBy
    order_set10.count += updateBy
}
...
function incrementRandomSet() {
    let randomSetNumber = getRandomInt()
    switch (randomSetNumber) {
        case 1: order_set1.count += 1; break;
        case 2: order_set2.count += 1; break;
        case 3: order_set3.count += 1; break;
        case 4: order_set4.count += 1; break;
        case 5: order_set5.count += 1; break;
        case 6: order_set6.count += 1; break;
        case 7: order_set7.count += 1; break;
        case 8: order_set8.count += 1; break;
        case 9: order_set9.count += 1; break;
        case 10 : order_set10.count += 1;
    }
}

function addCheckBoxesListeners() {
    var checkbox_x2 = document.getElementById("checkbox_multiply2");
    var checkbox_gift = document.getElementById("checkbox_getGift");
    checkbox_x2.addEventListener('change', function () {
        if (this.checked) {
            order_options[0].isActive = true;
            incrementSets(1);
            changeTotalPriceToValue(Math.round(order_cost * 1.6));
        } else {
    ...
    checkbox_gift.addEventListener('change', function () {
        let randomSetNum = getRandomInt()
        if (this.checked) {
            addOrderSet(randomSetNum)
            order_options[1].isActive = true;
        } else {
            removeOrderSet(randomNum)
...
window.onload = function () {
    allData = loadAllData();
    // fillOrganizationsTable(allData);
